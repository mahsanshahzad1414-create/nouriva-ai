...
with a:
    st.markdown(
        '<div class="eyebrow">NOURIVA • 2026</div>',
        unsafe_allow_html=True
    )
...
if st.session_state.page==PAGES[0]:
    ...
    st.markdown(
        """
        <div class="hero">
            <div class="eyebrow">STUDENT HEALTH-TECHNOLOGY PROTOTYPE</div>
            <h1>🌿 Nouriva AI</h1>
            ...
            in one premium student-built platform.
            </p>
            <span class="pill">9 CORE MODULES</span>
            <span class="pill">12+ EDUCATION TOPICS</span>
            <span class="pill">INTERACTIVE</span>
            <span class="pill">SDG-AWARE</span>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    mods=[
        ("🔍","Nutrition Scan","BMI + preliminary nutrition indicators.",1),
        ("🍽️","Diet Planner","Example meal plans around a general goal.",2),
        ("🤖","Nutrition Coach","General nutrition education assistant.",3),
        ("📚","Education","Lessons, key points and quizzes.",4),
        ("📊","Insights","Charts, indicators and SDG connections.",5),
        ("🧒","Growth Monitor","Growth-assessment concepts.",6),
        ("📷","Food Scanner","Image upload and workflow.",7),
        ("🌍","Global Nutrition","Major global nutrition challenges.",8),
        ("📄","Health Report","Downloadable educational summary.",9)
    ]
    ...
    for c,v,l in zip(
        cols,
        ["9","12+","5","LIVE"],
        ["Core modules","Education topics","SDG connections","Prototype status"]
    ):
        with c:
            st.markdown(
                f"""
    ...
    st.markdown(
        ...
        <div class="good">
            ...
            It is an educational prototype, not a diagnostic medical system.
        </div>
        """,
        unsafe_allow_html=True
    )
...
# ---------- SCAN ----------
    ...
    st.markdown(
        """
        <div class="module">
            <div class="eyebrow">SCREENING MODULE</div>
            <h1>🔍 Nutrition Scan</h1>
            <p>Calculate BMI and explore basic nutrition-related indicators.</p>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    with st.form("scan"):
        ...
        with c2:
            h=st.number_input("Height (cm)",30.,250.,170.)
            w=st.number_input("Weight (kg)",1.,300.,55.)
            fruit=st.selectbox(
    ...
    if st.session_state.screening:
        ...
        for c,label,val in [
            ...
            with c:
                st.markdown(
                    ...
                    <div class="metric">
                        <small>{label}</small>
                        <strong style="font-size:22px">{val}</strong>
                    </div>
                    """,
                    unsafe_allow_html=True
                )
        ...
        if x["age"]>=18:
            ...
            st.markdown(
                f"""
                <div class="{"warn" if x["bmi"]<18.5 or x["bmi"]>=30 else "good"}">
                    <b>{cat}</b><br>
                    Adult BMI screening interpretation.
                </div>
                """,
                unsafe_allow_html=True
            )
            ...
            st.markdown(
                """
                <div class="neutral">
                    <b>Age-specific interpretation</b><br>
        ...
        if x["factors"]:
            ...
            st.success("No basic risk indicators were identified.")
...
# ---------- DIET ----------
    ...
    st.markdown(
        ...
        <div class="module">
            ...
            <h1>🍽️ Diet Planner</h1>
            <p>Create practical example meals around a general goal.</p>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    with c2:
        veg=st.checkbox("Vegetarian pattern")
        budget=st.checkbox("Budget-friendly substitutions")

    if st.button(
        "🍽️ Generate My Example Plan",
        use_container_width=True
    ):
        st.session_state.plan=meal_plan(
            goal,
            veg,
            budget
        )
    ...
    if st.session_state.plan:
        ...
        st.markdown(
            """
            <div class="good">
                <b>Plan generated successfully.</b><br>
                Educational example based on your selected goal.
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            """
            <div class="neutral">
                These are example meal ideas, not individualized medical diets.
                Allergies, conditions and energy requirements require professional assessment.
            </div>
            """,
            unsafe_allow_html=True
        )
...
# ---------- COACH ----------
    ...
    st.markdown(
        """
        <div class="module">
            <div class="eyebrow">EDUCATION ASSISTANT</div>
            <h1>🤖 Nutrition Coach</h1>
            <p>Ask a general nutrition education question.</p>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    st.caption(
        "Try: protein • iron • hydration • BMI • fiber • calcium • food safety • balanced diet"
    )

    q=st.text_input(
        "Your question",
        placeholder="Why is protein important?"
    )
    ...
    if st.button(
        ...
        st.session_state.coach.append(
            (q,coach_answer(q))
        )

    for q,a in reversed(
        st.session_state.coach[-8:]
    ):
        ...
        st.markdown(
            ...
            <div class="panel">
                <b>YOU</b><br>
                {q}<br><br>
                <b style="color:#5ff0a8">🌿 NOURIVA COACH</b><br>
                {a}
            </div>
            """,
            unsafe_allow_html=True
...
# ---------- EDUCATION ----------
    ...
    st.markdown(
        ...
        <div class="module">
            ...
            <p>Structured lessons, key points and knowledge checks.</p>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    if st.button(
        ...
        ok=opts.index(selected)==ans

        st.session_state.quiz[topic]=ok

        if ok:
            st.success("✅ Correct — excellent.")
        else:
            st.error(
                f"❌ Not quite. Correct answer: {opts[ans]}"
            )

    if topic in st.session_state.quiz:

        st.metric(
            "Latest result",
            "1 / 1" if st.session_state.quiz[topic]
            else "0 / 1"
      )# ---------- INSIGHTS ----------
    ...
    st.markdown(
        """
        <div class="module">
            <div class="eyebrow">DATA & AWARENESS</div>
            <h1>📊 Nutrition Insights</h1>
            <p>Explore indicators, charts and SDG connections.</p>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    if choice=="Adult BMI screening concepts":
        ...
        st.info(
            "Adult BMI categories are screening concepts, "
            "not a complete health diagnosis."
        )
        ...
        st.success(
            "Nouriva connects nutrition awareness with Zero Hunger, "
            "Good Health, Quality Education, sustainability and poverty reduction."
        )
...
# ---------- GROWTH ----------
    ...
    st.markdown(
        """
        <div class="module">
            <div class="eyebrow">GROWTH AWARENESS</div>
            <h1>🧒 Growth Monitor</h1>
            <p>
                Educational demonstration of measurements used
                in child growth assessment.
            </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if st.session_state.growth:
        ...
        st.markdown(
            """
            <div class="neutral">
                <b>ℹ️ Interpretation</b><br>
                BMI alone cannot determine whether a child is growing normally.
                Proper assessment requires validated age- and sex-specific growth
                references and professional interpretation.
            </div>
            """,
            unsafe_allow_html=True
        )
...
# ---------- FOOD SCANNER ----------
    ...
    st.markdown(
        """
        <div class="module">
            <div class="eyebrow">IMAGE WORKFLOW</div>
            <h1>📷 Food Scanner</h1>
            <p>
                Upload an image to demonstrate Nouriva’s food-analysis workflow.
            </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if up:
        ...
        st.markdown(
            """
            <div class="good">
                <b>✓ Image received successfully</b><br>
                This prototype receives and inspects the image.
                Reliable food recognition and nutrient estimation would require
                a trained model and validated database.
            </div>
            """,
            unsafe_allow_html=True
        )
...
# ---------- GLOBAL ----------
    ...
    st.markdown(
        """
        <div class="module">
            <div class="eyebrow">GLOBAL AWARENESS</div>
            <h1>🌍 Global Nutrition</h1>
            <p>
                Explore major nutrition challenges and development connections.
            </p>
        </div>
        """,
        unsafe_allow_html=True
...
# ---------- REPORT ----------
    ...
    st.markdown(
        ...
        <div class="module">
            ...
            <h1>📄 Health Report</h1>
            <p>
                Generate an educational summary from the latest screening.
            </p>
        </div>
        """,
        unsafe_allow_html=True
...
"""
        ...
        st.download_button(
            ...
            report,
            "Nouriva_AI_Nutrition_Report.txt",
            "text/plain",
            use_container_width=True
        )
...
# ---------- ABOUT ----------
    ...
    st.markdown(
        ...
        <div class="module">
            ...
            <h1>ℹ️ About Nouriva AI</h1>
            <p>
                A student-built nutrition and health-technology prototype.
            </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    with c1:
        ...
        st.markdown(
            ...
            <div class="card">
                <h3>🌿 Mission</h3>
                <p>
                    Make nutrition awareness easier to explore through screening,
                    education, planning and accessible digital tools.
                </p>
            </div>
            """,
            unsafe_allow_html=True
    ...
    with c2:
        ...
        st.markdown(
            ...
            <div class="card">
                <h3>🚀 Scope</h3>
                <p>
                    Nouriva demonstrates interactive nutrition workflows while
                    separating educational features from clinical diagnosis.
                </p>
            </div>
            """,
            unsafe_allow_html=True
    ...
    st.write("**Muhammad Ahsan Shahzad**")
    ...
    st.markdown(
        """
        <div class="warn">
            <b>⚠️ Educational prototype</b><br>
            Nouriva AI does not diagnose disease, prescribe treatment
            or replace qualified healthcare professionals.
        </div>
        """,
        unsafe_allow_html=True
    )
...
st.markdown(
    ...
    <div class="footer">
        ...
        Student Health-Technology Prototype • 2026<br>
